...
function getCia($dba) {
    $sql = "SELECT id,razonsocial,ruc,alias
            FROM cia
            ";
...
function regUser($dba) {
    $cia = $_POST['cia'];
    $usuario = strtoupper($_POST['usuario']);
    $contraseña = $_POST['contraseña'];
    $nombre = strtoupper($_POST['nombre']);
    $nrodocu = $_POST['nrodocu'];
    $confirma_contraseña = $_POST['confirma_contraseña'];
    $apellidos = strtoupper($_POST['apellidos']);
    $telefono = $_POST['telefono'];
    $correo = $_POST['correo'];
    $hashedPassword = password_hash($contraseña, PASSWORD_DEFAULT); // Encriptar la contraseña
    
    // Consulta para insertar los datos
    $sql = "INSERT INTO user (idcia,user, `password`, nombre, apellidos, nrodocu, telefono, correo) 
            VALUES ($cia,'$usuario','$hashedPassword','$nombre','$apellidos','$nrodocu','$telefono','$correo')";
    //echo '<pre>';print_r($sql);echo '</pre>';die;
    $stmt = $dba->prepare($sql);
    
    if ($stmt->execute()) {
        $ult_id = $dba->getLastInsertId();
        echo json_encode(['status' => 'success', 'message' => 'Registro Usuario exitosamente', 'ult_id' => $ult_id]);
    } else {
        if($stmt->errno === 1062){
            echo json_encode(['status' => 'error', 'message' => 'El usuario ya ha sido registrado']);
        }else{
...
function validlogin($dba) {
    $cia = $_POST['cia'];
    $usuario = strtoupper($_POST['usuario']);
    $logincontraseña = $_POST['logincontraseña'];
    
    // Consulta para insertar los datos
    $sql = "SELECT * FROM user WHERE idcia=$cia AND user='$usuario' AND `status`='' OR `status` IS NULL";
    
    $rpt = $dba->rptArray($sql);

    if ($rpt) {
        $sql = "SELECT id,`password` FROM user WHERE idcia=$cia AND user='$usuario' AND `status`='' OR `status` IS NULL";
        $rpt = $dba->rptArray($sql);
        $validpass=password_verify($logincontraseña,$rpt[0]['password']);
        $id=$rpt[0]['id'];
        if($validpass){
            // Obtener la IP del servidor
            //$serverIP = $_SERVER['SERVER_ADDR'];
            //$url="$serverIP/comercial/main/home.php";

            echo json_encode(['status' => 'success','cia' => $cia,'id' => $id]);
        }else{
            echo json_encode(['status' => 'error', 'message' => 'Usuario y Contraseña incorrecta']);
        }
    }else{
